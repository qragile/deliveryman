...
export const createScanner = (scanner) => async (dispatch, getState) => {
...
export const listScanners = () => async (dispatch, getState) => {
  dispatch({ type: SCANNER_LIST_REQUEST });
  const {
    userSignin: { userInfo },
  } = getState();
  try {
    ...
    dispatch({ type: SCANNER_LIST_SUCCESS, payload: data });
  } 
    catch (error) {
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
    dispatch({ type: SCANNER_LIST_FAIL, payload: message });
  }
};
